...
> 
> 정보 카드를 제작할 때 전처리된 텍스트 자체가 너무 길어서 프롬프트의 제약사항을 잊어버리는 문제가 생기고 이를 해결하기 위해서 정책의 핵심 내용을 추출하고, 기사의 핵심 내용을 추출하는 노드를 추가했습니다. 핵심 분량이 아직 1000자 내외이기 때문에 분량을 늘릴 방안을 늘리는 쪽을 생각 중입니다. 정책 카드에서의 찬반 의견을 보여줄 때 찬성 측 의견은 다양하게 나오지만 반대측 의견에 있어 ‘근본적 해결이 안 됨’, ‘사각지대 존재’ 등 비슷한 어구가 출력됩니다. 이를 해결하기 위해 검색 툴을 달아줬지만 아직 고도화가 필요합니다.

## 10-4. 채팅
> 카드 추천, 사용자가 카드를 선택하고 쿼리를 입력했을 때 해당 카드와 관련이 있는지, 혹은 새로운 카드를 추천해야하는지, 추천을 한다면 어떤 카드를 추천해야 하는지를 멀티 에이전트로 변환하였습니다.

## 10-5. 토론
> 찬성 및 반대에서 간단한 RAG 검색 및 출처를 활용한 답변 생성을 구현하였으며, AI vs User 모드에서 금칙어 및 주제와 관련 없는 답변에 대한 필터링을 적용하였습니다.
찬성·반대 에이전트 고도화를 위해 근거 검토 에이전트를 활용하여 "발언 안의 수치 및 사실이 검색된 문서와 일치하는가", "발언에 출처로 사용할 수 있는 근거가 충분히 존재하는가"를 확인하는 기능을 구현하였습니다.
> 
> 가능한 경우 AI vs User 모드에서 User Assistant를 구현하여 통계 자료 추천 및 주장의 논리적 구조 검토 기능을 제공할 예정입니다.
추가적으로 가능한 경우 TTS / STT 기능 구현을 검토하고 있습니다.
