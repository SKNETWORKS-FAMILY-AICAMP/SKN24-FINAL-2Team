...
> 
> 정보 카드를 제작할 때 전처리된 텍스트 자체가 너무 길어서 프롬프트의 제약사항을 잊어버리는 문제가 생기고 이를 해결하기 위해서 정책의 핵심 내용을 추출하고, 기사의 핵심 내용을 추출하는 노드를 추가했습니다. 핵심 분량이 아직 1000자 내외이기 때문에 분량을 늘릴 방안을 늘리는 쪽을 생각 중임. 정책 카드에서의 찬반 의견을 보여줄 때 찬성 측 의견은 다양하게 나오지만 반대측 의견에 있어 ‘근본적 해결이 안 됨’, ‘사각지대 존재’ 등 비슷한 어구가 나옴. 이를 해결하기 위해 검색 툴을 달아줬지만 아직 고도화가 필요합니다.
