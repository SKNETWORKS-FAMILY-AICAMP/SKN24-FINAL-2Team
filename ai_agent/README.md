...
`.env` 파일을 프로젝트 루트(`ai_agent/`)에 생성합니다.
...
# Docker Compose 실행 시 (서비스명 사용)
# QDRANT_URL=http://qdrant:6333

# ── LangGraph Checkpoint ──────────────────────────────────────────────────────
# 로컬 실행 시
CHECKPOINT_PATH=./checkpoints/debate.db
# Docker Compose 실행 시
# CHECKPOINT_PATH=/app/checkpoints/debate.db

# ── 임베딩 모델 ───────────────────────────────────────────────────────────────
EMBED_MODEL_KEY=ko-sroberta
```
...
> Qdrant를 docker-compose로 띄운 뒤 ai_agent FastAPI 서버를 실행합니다.

```bash
# 1. Qdrant 실행 (docker-compose 사용 — 데이터 영구 보존)
cd ai_agent
docker-compose up -d qdrant
```

> ⚠️ **`docker run` 대신 반드시 `docker-compose up -d qdrant`를 사용하세요.**  
> `docker run`으로 실행하면 컨테이너 재시작 시 벡터 데이터가 모두 사라집니다.  
> docker-compose는 `./qdrant_storage:/qdrant/storage` 바인드 마운트로 데이터를 영구 보존합니다.

```bash
# 2. 최초 실행 시에만: 노란봉투법 뉴스 데이터 업로드
python upload_yellow_envelope.py
```

> ✅ `qdrant_storage/` 디렉토리에 데이터가 이미 있으면 이 단계는 건너뜁니다.
...
> ⚠️ **`qdrant-client==1.9.2`** 를 사용해야 합니다.  
> Qdrant 서버 v1.9.0과 클라이언트 v1.10+ 이상은 호환되지 않습니다 (`/query` API 미지원).  
> requirements.txt에 `qdrant-client==1.9.2`가 명시되어 있는지 확인하세요.
...
서버 기동 후 로그에서 아래 메시지가 모두 출력되면 준비 완료입니다.

```
ko-sroberta 임베딩 모델 로드 완료
Qdrant 연결: http://localhost:6333
LangGraph 그래프 빌드 완료
```

---

### 방법 2 — Docker Compose (운영)
...
`docker-compose.yml`이 ai_agent + Qdrant 컨테이너를 함께 실행하며, LangGraph checkpoint와 Qdrant 스토리지는 named volume으로 영구 보존됩니다.

| 서비스 | 포트 | 설명 |
|--------|------|------|
| `ai_agent` | 8001 | FastAPI 토론 서버 |
...
서버 실행 후 `http://localhost:8001/docs` 에서 Swagger UI를 확인할 수 있습니다.
