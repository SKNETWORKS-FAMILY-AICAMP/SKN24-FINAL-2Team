...
from config import GOV24_API_KEY, OPENAI_API_KEY, OUTPUT_DIR, DATA_DIR, REQUEST_TIMEOUT
...
CATEGORIES = ["일자리", "주거", "금융", "복지", "교육문화", "참여"]
TOP_N      = 5
...
class Gov24Collector:
    ...
    def collect_and_process(
        ...
        """
        youth_csv = DATA_DIR / "metadata" / "gov24_youth.csv"
        seen_csv  = DATA_DIR / "metadata" / "gov24_seen.csv"
        ...
        for cat in CATEGORIES:
            ...
            categorized[cat] = [
                {**{k: v for k, v in item.items() if not k.startswith("_")},
                 "_importance": item["_importance"]}   # csv 저장용으로 유지
                for item in top5
            ]
    ...
    def _gpt_categorize_batch(self, rows: List[dict]) -> List[dict]:
        ...
        system_prompt = (
            ...
            "  금융:     대출, 저축, 금리우대, 신용지원, 장학금(생활비)\n"
            "  복지:     의료, 심리상담, 생활지원, 긴급복지, 사회서비스\n"
            "  교육문화: 교육비지원, 자격증, 문화·여가, 도서관, 예술\n"
    ...
    def _save(self, items: List[dict], tag: str = "") -> None:
        ...
        json_path = OUTPUT_DIR / f"gov24정책{tag_str}_{ts}.json"
        csv_path  = OUTPUT_DIR / f"gov24정책{tag_str}_{ts}.csv"
    ...
    def _save_categorized(self, categorized: Dict[str, List[dict]]) -> None:
        """카테고리 Top5를 data/metadata/gov24_top5_날짜.csv로 저장"""
        META_DIR = DATA_DIR / "metadata"
        META_DIR.mkdir(parents=True, exist_ok=True)

        today     = datetime.now().strftime("%Y-%m-%d")
        csv_path  = META_DIR / f"gov24_top5_{today}.csv"
        json_path = META_DIR / f"gov24_top5_{today}.json"
        ...
        logger.info(f"[Gov24] 카테고리 Top5 저장 → {csv_path.name}, {json_path.name} (총 {total}건)")
        for cat, items in categorized.items():
            logger.info(f"  [{cat}] {len(items)}개")
            for rank, item in enumerate(items, 1):
                logger.info(f"    {rank}. {item.get('서비스명', '')}")